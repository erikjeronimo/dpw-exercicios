...
Comando utilizado para criar o commit de teste:
...
```powershell
git commit -m "docs: teste do caso 4"
```
...
O `git restore` foi utilizado para descartar alteracoes locais, enquanto `git restore --staged` foi utilizado para retirar uma alteracao do staging sem apaga-la.
